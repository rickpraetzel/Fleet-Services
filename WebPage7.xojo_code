...
Begin WebPage WebPage7
   ...
   Begin AutoDiscoveryClass AutoDiscoveryClass1
      ...
      Top             =   0.0
   End
   Begin TirrerotationDialog TirrerotationDialog1
      Cursor          =   0
      Enabled         =   True
      Height          =   78
      HelpTag         =   ""
...
#tag WindowCode
	#tag Event
		Sub Open()
		  ...
		  TextField4.text = datetohrdate(d)
		  ...
		  if session.mysqldb <> nil then
		    if session.mysqldb.Connect then
		      if popupmenu1.text <> "Vehicle..." AND popupmenu2.text <> "Driver..." then 'we have a report to record
		        if TextField1.text = "" or textfield4.text = "" then
		          MsgBox "Please provide the mileage and date."
		          exit
		        end if
		        
		        dim rs as RecordSet
		        dim sqlstring,vserial,s as string
		        dim rec as DatabaseRecord
		        dim d as new date
		        
		        vserial = PopupMenu1.RowTag(PopupMenu1.ListIndex)
		        sqlstring = "SELECT * from vehicles where serial = " + vserial
		        rs = session.mysqldb.SQLSelect(sqlstring)
		        if rs <> nil then
		          if not rs.bof and not rs.eof then
		            rs.edit
		            s = hrdatetoworddate(textfield4.text)
		            rs.field("lastoilchangedate").value = s
		            rs.field("lastoilchangemileage").value = textfield1.text
		            '-----
		            if didtirerotation then
		              rs.field("lasttirerotationmileage").value = textfield1.text
		            end if
		            rs.update
		            if Session.mysqldb.ErrorCode = 0 then
		              ...
		              MsgBox  "Failed to record this fillup."
		            end if
		          else
		            MsgBox "Could not find this vehicle in the database."
		          end if
		        end if
		      end if
		    end if
		    didtirerotation = false
		  end if
...
#tag Events ImageView1
	#tag Event
		Sub MouseUp(X As Integer, Y As Integer, Details As REALbasic.MouseEvent)
		  WebPageSeekingDate = 7
		  WebCalendar1.show
		  WebCalendar1.settingdate = 0
		End Sub
	#tag EndEvent
#tag EndEvents
#tag Events TirrerotationDialog1
	#tag Event
		Sub Dismissed()
		  RecordTheOilChange
		End Sub
